...
## TASKS:
- [x] REQUEST SERVICE ACCOUNT KEY: Generate a fresh JSON key from Firebase Project Settings → Service Accounts.
- [x] SECURE KEY STORAGE: Move the JSON outside the repo, lock down OS permissions, and document its absolute path.
- [x] UPDATE CONFIGURATION: Add an environment variable (e.g., `FIREBASE_CREDENTIALS`) that points to the stored key path.
## FILES:
...
## TASKS:
- [x] INSTALL DEPENDENCIES: Add `firebase-admin` and the chosen server framework (e.g., Express) to `package.json`.
- [x] INITIALIZE ADMIN SDK: Create shared bootstrap code that loads the service-account key and configures the Firestore client.
- [x] CREATE STATUS ENDPOINT: Build `/api/status` that responds only after a Firestore ping succeeds.
## FILES:
...
## TASKS:
- [x] CREATE SESSION API: Add the `/api/session/start` route to `server/index.js`.
- [x] WRITE INITIAL SESSION DOC: Use Firestore to insert the new document with timestamps, `userData`, and `productChoice`.
- [x] RETURN SESSION ID: Send the document ID back to the client so the iframe query string and postMessage can reuse it.
## FILES:
...
## TASKS:
- [x] DEFINE FACET STRUCTURE: Document exact keys and acceptable values that the API will accept.
- [x] IMPLEMENT SAVE ENDPOINT: Add a route on the server that validates the payload and updates Firestore with atomic merges.
- [x] HOOK UP CLIENT CALLS: Update `scripts/app.js` so every facet change triggers the save endpoint before the UI rerenders.
## FILES:
...
## TASKS:
- [x] CREATE USER-DATA ENDPOINT: Merge `userName`, `userPhone`, `userEmail`, and `talkToHuman` into the session document.
- [x] CREATE MESSAGE ENDPOINT: Append message bubbles to a `messages` subcollection with timestamps and `role`.
- [x] UPDATE CHAT FLOW: Modify `scripts/ui/chat.js` (and any Gemini orchestrator) to call these endpoints after every send/receive event.
## FILES:
